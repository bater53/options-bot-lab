"""
LAB: Project SPY double-bottom underlying trades into option trade plans.

Usage:
    python -m src.run_double_bottom_spy_options_lab
"""
    ...
    *,
    ...
    Rule (LAB version):
      ...
      - Strike: slightly OTM by `otm_pct` (%) from the underlying entry price,
                rounded to a SPY/QQQ-style strike increment.
    """
...
# ── SPY double-bottom core (copied from your LAB runner) ──────
...
def load_spy_5m(period: str = "60d") -> pd.DataFrame:
    df = yf.download(
        "SPY",
        interval="5m",
        period=period,
    ...
    if df is None or df.empty:
        raise RuntimeError("Failed to download SPY data (empty dataframe).")
...
# ── Map SPY underlying trades to option plans ─────────────────

def underlying_trade_to_signal(
    df: pd.DataFrame,
    trade: PlannedTrade,
    rule_id: str = "DB_SPY_5M_LAB_V1",
) -> UnderlyingSignal:
    """
...
def build_option_plans_for_spy() -> List[OptionTradePlan]:
    period = "60d"
    print(f"Loading SPY 5m candles for about the last {period}…")
    df = load_spy_5m(period)
...
def main() -> None:
    plans = build_option_plans_for_spy()
    
    print()
    print("=== SPY Double-Bottom → Call Option LAB Plans ===")
    ...
    out_path = "data/lab_spy_double_bottom_options.csv"
    df_export.to_csv(out_path, index=False)
    print(f"\n[LAB] Exported SPY option plans to {out_path}")
